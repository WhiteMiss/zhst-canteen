		...
		<div class="shopping-car-list-box">
			...
			<mu-slide-bottom-transition>
				<div class="car-container" v-show="show">
					...
					<div class="null-item" v-if="CarInfo.good_count == 0">空空如也~</div>
					<div class="list-box">
						<div class="list-item" v-for="(item, index) in CarInfo.goods">
							<div class="name-info">
								<div class="name">{{item.name}}</div>
								<div class="specs">{{item.specsname}}</div>
							</div>
							<div class="choose-number">
								<div class="choose-btn minus-btn" @click="AddMinus('minus', item.thisid)">
									<span class="iconfont">&#xe601;</span>
								</div>
								<div class="choose-btn number-btn">{{item.count}}</div>
								<div class="choose-btn add-btn" @click="AddMinus('add', item.thisid)">
									<span class="iconfont">&#xe600;</span>
								</div>
							</div>
							<div class="price">￥<span class="price-number">{{(item.price * item.count) | SolveAccuracy}}</span></div>
						</div>
						<div class="boxFee" v-if="CarInfo.boxFee.count > 0">
							餐盒 ×{{CarInfo.boxFee.count}}
							<span class="money">￥<span>{{CarInfo.boxFee.money}}</span></span>
						</div>
					</div>
				</div>
			</mu-slide-bottom-transition>
...
<script>
	...
	export default {
		...
		methods: {
			...
			Clear() {
				this.ClearCar(this.canteenid);
				this.openAlert = false;
			},
			AddMinus(type, thisid) {
				if(type == 'add' && this.CarInfo.good_count >= 6) {
					this.$toast('购物车最多只能有6件商品');
					return;
				}
				this.AddMinusCount({ type, canteenid: this.canteenid, thisid })
			},
			...SaveListMutations(['ClearCar', 'AddMinusCount'])
		},
		computed: {
			canteenid() {
				return this.$route.params.canteenid;
			},
			CarInfo() {
